...
/** The closed rectangle a ring occupies: five points, wound consistently, still fillable. */
function boundingQuad(points) {
 ...
 *
 * `collapse` changes what happens when a closed ring simplifies away to nothing, and it is
 * off by default because the fallback below is load-bearing for the callers that don't set
 * it. A ring reduced under four points is no longer a polygon, so the default is to hand
 * back the *original* — correct, but it means the hardest-simplified rings are the ones
 * that keep every vertex. At a wide zoom that inverts the whole point of simplifying: a
 * zone smaller than a pixel is exactly the one that collapses, and it comes back at full
 * resolution. In `public/zones/US.json` that put 88% of 689,078 points into 10% of the
 * zones, one of them 14,243 points for a shape covering about a pixel.
 *
 * With `collapse`, such a ring becomes its own bounding quad instead: five points, visually
 * identical at the zoom that collapsed it, and still a fillable polygon.
 */
export function simplify(points, tolerance, closed, collapse) {
  ...
  if (out.length < 4) return collapse ? boundingQuad(points) : points;
  const [fx, fy] = out[0];
...
export function prepare(coordinates, shift, tolerance, closed, factor, collapse) {
  const walk = (node) => {
    if (typeof node[0][0] === "number") {
      const shifted = node.map(([lon, lat]) => [shift(lon), lat]);
      return round(simplify(shifted, tolerance, closed, collapse), factor);
    }
