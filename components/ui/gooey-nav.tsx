...
// `auto` follows the OS colour scheme, as upstream. A bar that always sits on a dark (or
// light) surface should pin its tone instead, or it flips with a setting the page ignores.
const TONES = {
  auto: { bar: "bg-[#F4F4F9] dark:bg-[#262626]", barText: "text-[#F4F4F9] dark:text-[#262626]" },
  light: { bar: "bg-[#F4F4F9]", barText: "text-[#F4F4F9]" },
  dark: { bar: "bg-[#262626]", barText: "text-[#262626]" },
} as const;

export type GooeyNavTone = keyof typeof TONES;
...
type NavItem = {
  ...
  href?: string;
  icon?: ReactNode;
  /** Rendered after the label, inside the item — a badge, say. */
  suffix?: ReactNode;
  /** `data-*` attributes for the item's button or link, for tests and automation to target. */
  attributes?: Record<`data-${string}`, string | number | undefined>;
};
